...
from prompt_condense_captions import examples, instruction
##############################################################
...
def process_outputs(inference_outputs, captions, output_path):
    ...
    for i in tqdm(range(len(inference_outputs))):
        ...
        condensed_caption = response.outputs[0].text
   
        res.append({'audio_id': audio_id, 'original_caption': orginial_caption, 'condensed_caption': condensed_caption})
    
    
    # save to csv file
    with open(output_path, 'w', newline='', encoding='utf-8') as f:
       
        fieldnames = ['audio_id', 'original_caption', 'condensed_caption']
...
if __name__ == "__main__":    
   ...
   # run Mixtral inference
    ...
    # store LLM outputs
    output_path = 'datasets/Clotho_LARCQ/condensed_captions.csv'
