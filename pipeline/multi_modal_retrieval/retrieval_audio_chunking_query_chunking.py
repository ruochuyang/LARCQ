...
# load in audios and captions
    ...
    split_captions = pd.read_csv(data_path).to_dict('records')
...
def compute_score_audio_chunk(text_embedding, audio_chunks_embeddings):
    ...
    audios_score = np.array(audios_score)
...
def compute_metrics(split_captions, audio_chunks_embeddings, split_text_embeddings):
# compute R@ metrics for the split captions
    ...
    for idx in tqdm(range(len(split_text_embeddings))):

        query_embeddings = split_text_embeddings[idx]
 
        scores = []
        
        for split_embedding in query_embeddings:
            
            audios_score = compute_score_audio_chunk(split_embedding, audio_chunks_embeddings)

            scores.append(audios_score)
...
def evaluate(benchmark, result_path): 
    ...
    audio_embedding_path = f'datasets/{benchmark}/features/audio-embeddings-best.pkl'
    ...
    ranking = compute_metrics(split_captions, audio_chunks_embeddings, split_text_embeddings)
    
    # save rank list to a pickle file
    ranking_path = f'./datasets/{benchmark}/clap_ranking/audio-ranking-best.pkl'
...
if __name__ == '__main__':
    ...
    result_path = f'results/retrieved_results/{benchmark}/retrieved_audios_best.csv'
